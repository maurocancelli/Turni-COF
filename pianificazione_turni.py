...
def genera_tabellone(week_num: int, anno: int, lunedi: datetime.date,
    ...
    for dip in dati:
        ...
        if data_mal is not None:
            try:
                data_mal = pd.to_datetime(data_mal).date()
            except Exception:
                data_mal = None
...
with tab_turni:
    ...
    # Costruisci label per ogni tab settimana
    labels_week = []
    for anno_w, week_w, lun_w in settimane:
        ...
        is_current = (anno_w == anno_corrente and week_w == week_corrente)
    ...
    for i, (t_week, (anno_w, week_w, lun_w)) in enumerate(zip(tabs_week, settimane)):
        with t_week:
            ...
            data_dom_p_w = lun_w - datetime.timedelta(days=1)
            data_dom_s_w = lun_w + datetime.timedelta(days=6)

            # Costruisci label colonne con data
            col_labels = {}
            rinomina_exp = {}
            ...
            if df_salvato is not None:
                ...
                # Aggiorna Dom_P dalla settimana precedente se non è la prima tab
                if i > 0:
                    anno_prec_w, week_prec_w, lun_prec_w = settimane[i - 1]
                    df_prec_w = carica_settimana(anno_prec_w, week_prec_w)
                ...
                # Se settimana precedente è nella lista tabs, usa i dati già calcolati
                if i > 0:
                    anno_prec_w, week_prec_w, lun_prec_w = settimane[i - 1]
                    df_prec_w = carica_settimana(anno_prec_w, week_prec_w)
                    if df_prec_w is not None and "Dom_S" in df_prec_w.columns:
            ...
            with col1:
                if not definitiva:
                    if st.button(f"🔒 Blocca come Definitiva", type="primary",
                                 use_container_width=True, key=f"blocca_{anno_w}_{week_w}"):
                        salva_settimana(df_modificato, anno_w, week_w, definitiva=True)
                        st.success("✅ Settimana bloccata come definitiva!")
                        st.rerun()
                else:
                    if st.button(f"🔓 Sblocca (torna Provvisoria)", type="secondary",
                                 use_container_width=True, key=f"sblocca_{anno_w}_{week_w}"):
                        salva_settimana(df_modificato, anno_w, week_w, definitiva=False)
                        st.success("↩️ Settimana riportata a provvisoria!")
                        st.rerun()

            with col2:
                if st.button(f"💾 Salva Modifiche", use_container_width=True,
            ...
            st.dataframe(
                df_modificato.style.map(colora_celle),
                use_container_width=True
            )
